...
class SccSmoother:
  ...
  # accel_gain, decel_gain, curvature_gain
  def __init__(self):
      ...
      if 0. < d < -lead.vRel * (7.7 + cruise_gap) * 2. and lead.vRel < -1.:
        ...
        if 40 > lead.dRel > 12 and clu11_speed < 15.0 * CV.MS_TO_KPH:
          ...
          accel = op_accel * interp(clu11_speed, [0., 30., 38., 50., 51., 60., 100.],
                                    [2.3, 3.4, 3.2, 1.7, 1.65, 1.4, 1.25])

    if accel > 0.:
      accel *= gas_factor * interp(clu11_speed, [35., 60., 100.], [1.7, 1.42, 1.35])
    else:
      accel *= brake_factor * 1.8
